...
import { ArrowUpRight, Check, ChevronDown, Facebook, Globe, Instagram, Linkedin, LockKeyhole, Mail, Menu, Phone, X } from 'lucide-react';
import './index.css';
import './brand.css';
...
type Flow = 'landing' | 'register' | 'payment' | 'success' | 'terms' | 'privacy';
...
const whatsappWorkshopUrl = 'https://chat.whatsapp.com/KbbfA9MHHNi3FZnEsf4iTf';
...
function App() {
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      let verification: { paid: boolean; status?: string } = { paid: false };
      for (let attempt = 0; attempt < 15; attempt += 1) {
        ...
        if (verificationResponse.ok && verification.paid) break;
        if (attempt < 14) await new Promise(resolve => window.setTimeout(resolve, 2000));
      }
      if (!verification.paid) throw new Error(`Payment is not confirmed yet (status: ${verification.status || 'unknown'}). Please try again after checking your bank or UPI app.`);
      window.location.assign(whatsappWorkshopUrl);
...
function Landing({ onEnroll, menu, setMenu }: { onEnroll: () => void; menu: boolean; setMenu: (v: boolean) => void }) { return <div id="top"><Header onEnroll={onEnroll} menu={menu} setMenu={setMenu} /><main>
  <section className="hero"><div className="hero-copy"><p className="eyebrow">Six-day intensive <span /> Online workshop</p><h1>Design in<br /><em>Generative AI</em><br /><span>& research in LLM</span></h1><p className="hero-sub">Design smarter. Research deeper.<br />Create impact.</p><div className="hero-actions"><button className="button button-gold" onClick={onEnroll}>Enroll now <Price /></button><a className="text-link" href="#days">View workshop <ArrowUpRight size={17} /></a></div></div><div className="hero-art"><div className="art-label">A new literacy<br /><strong>for the<br />design field</strong></div><div className="art-note">{workshopDates}</div></div><div className="hero-meta"><span>01</span><span>Design education for a changing practice</span><span>Scroll to explore ↓</span></div></section>
  <section className="intro section-pad"><div className="section-kicker">/ The proposition</div><div className="intro-grid"><h2>Where thoughtful<br /><span>design meets</span><br />machine intelligence.</h2><div className="intro-copy"><p>AI is not replacing the designer. It is expanding the designer's field of vision.</p><p>This six-day intensive, running from {workshopDates}, is a working introduction to the tools, methods and questions shaping the next chapter of creative practice.</p><a className="text-link dark-link" href="#learn">Read the curriculum <ArrowUpRight size={17} /></a></div></div></section>
  ...
  <section className="statement"><div className="statement-inner"><span className="section-kicker">/ A working six days</span><p>Less <i>prompting.</i><br />More <strong>thinking.</strong></p><span className="statement-side">{workshopDates}<br />A compact, rigorous<br />studio of ideas.</span></div></section>
  ...
  <section className="days section-pad" id="days"><div className="section-kicker">/ The six-day workshop · {workshopDates}</div><div className="days-head"><h2>Six days.<br /><span>Real momentum.</span></h2><p>Each day builds on the last, moving from design thinking to a complete, AI-assisted design story.</p></div><div className="day-list">{days.map(({ number, title, topics }) => <details key={number} open={number === '01'}><summary><span className="day-num">{number}</span><strong>{title}</strong><ChevronDown size={18} /></summary><ol className="day-topics">{topics.map(topic => <li key={topic}>{topic}</li>)}</ol></details>)}</div></section>
  <section className="offer"><div className="offer-copy"><div className="section-kicker">/ Special offer</div><h2>Your next<br /><em>six days</em><br />start here.</h2><p>One focused week to make the unfamiliar feel possible.</p></div><div className="offer-card"><span className="offer-tag">Limited seats · 100% online</span><small>JUST</small><Price dark /><div className="offer-rule" /><p>Includes all live sessions, project feedback, certificate of completion and community support.</p><button className="button button-navy" onClick={onEnroll}>Secure my seat <ArrowUpRight size={17} /></button></div></section>
...
</main><footer><div className="footer-brand"><Logo /><p>Designing Spaces Than Inspire Living</p></div><div className="footer-contact"><span className="footer-label">Contact</span><a href="tel:+918530863658"><Phone size={15} />8530863658</a><a href="mailto:contact@reeghdesign.com"><Mail size={15} />contact@reeghdesign.com</a></div><div className="footer-social"><span className="footer-label">Find RéEGH</span><div className="social-links"><a href="https://www.instagram.com/reegh.co/" target="_blank" rel="noreferrer" aria-label="RéEGH on Instagram"><Instagram size={18} /></a><a href="https://www.facebook.com/p/Re%C3%A9gh-and-Company-61569432482418/" target="_blank" rel="noreferrer" aria-label="RéEGH on Facebook"><Facebook size={18} /></a><a href="https://in.linkedin.com/company/reegh-and-company" target="_blank" rel="noreferrer" aria-label="RéEGH on LinkedIn"><Linkedin size={18} /></a><a href="https://www.reeghstudio.com/" target="_blank" rel="noreferrer" aria-label="RéEGH website"><Globe size={18} /></a></div><nav className="footer-legal"><a href="/terms">Terms</a><a href="/privacy">Privacy</a></nav></div><small>© 2026 RéEGH. All rights reserved.</small></footer><div className="mobile-cta"><button onClick={onEnroll}>Enroll now — ₹1,299 + GST <ArrowUpRight size={16} /></button></div></div>; }
...
function FlowPage({ flow, form, onChange, errors, onSubmit, onBack, onPay, paymentBusy, paymentError }: { flow: Flow; form: FormData; onChange: (k: keyof FormData, v: string | boolean) => void; errors: Partial<Record<keyof FormData, string>>; onSubmit: (e: React.FormEvent) => void; onBack: () => void; onPay: () => void; paymentBusy: boolean; paymentError: string }) { if (flow === 'success') return <div className="flow-shell"><Logo /><div className="success-box"><div className="success-icon"><Check /></div><p className="eyebrow">Payment successful</p><h1>You're officially<br /><em>registered.</em></h1><p>Your seat for the <strong>Design in Generative AI & Research in LLM</strong> 6-Day Intensive Workshop has been confirmed.</p><div className="receipt"><span>Participant name<strong>{form.fullName}</strong></span><span>Registration ID<strong>6D-{Math.floor(10000 + Math.random() * 89999)}</strong></span><span>Payment status<strong className="paid">PAID</strong></span></div><a className="button button-gold full" href={whatsappWorkshopUrl} target="_blank" rel="noreferrer">Join WhatsApp workshop group <ArrowUpRight size={17} /></a><p className="fine-print">Please join the WhatsApp group to receive workshop schedule, meeting links, announcements and important updates.</p><button className="receipt-link">Download / view payment receipt</button></div></div>;
 if (flow === 'payment') { const cashfreeEnvironment = import.meta.env.VITE_CASHFREE_ENV === 'production' ? 'production' : 'sandbox'; return <div className="flow-shell"><Logo /><div className="payment-box"><button className="back" onClick={() => onBack()}>← Back to registration</button><p className="eyebrow">Secure checkout · Cashfree {cashfreeEnvironment}</p><h1>Complete your<br /><em>enrollment.</em></h1><div className="pay-summary"><span>Six-day intensive workshop · {workshopDates}</span><strong><Price /></strong></div><div className="secure-line"><LockKeyhole size={16} /> Cashfree secure checkout · UPI · Cards · Net banking</div>{paymentError && <p className="payment-error">{paymentError}</p>}<button className="button button-navy full" onClick={onPay} disabled={paymentBusy}>{paymentBusy ? 'Opening Cashfree...' : 'Continue to Cashfree'} <ArrowUpRight size={17} /></button><p className="fine-print">Your payment is verified with Cashfree before registration is confirmed.</p></div></div>; }
