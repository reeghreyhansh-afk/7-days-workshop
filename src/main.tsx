...
function App() {
  const [flow, setFlow] = useState<Flow>('landing'); const [form, setForm] = useState(initialForm); const [errors, setErrors] = useState<Partial<Record<keyof FormData, string>>>({}); const [menu, setMenu] = useState(false);
  ...
  if (flow === 'register' || flow === 'payment' || flow === 'success') return <FlowPage flow={flow} form={form} onChange={update} errors={errors} onSubmit={submit} onBack={() => setFlow('landing')} onPay={() => setFlow('success')} />;
...
function FlowPage({ flow, form, onChange, errors, onSubmit, onBack, onPay }: { flow: Flow; form: FormData; onChange: (k: keyof FormData, v: string | boolean) => void; errors: Partial<Record<keyof FormData, string>>; onSubmit: (e: React.FormEvent) => void; onBack: () => void; onPay: () => void }) { if (flow === 'success') return <div className="flow-shell"><Logo /><div className="success-box"><div className="success-icon"><Check /></div><p className="eyebrow">Payment successful</p><h1>You're officially<br /><em>registered.</em></h1><p>Your seat for the <strong>Design in Generative AI & Research in LLM</strong> 7-Day Intensive Workshop has been confirmed.</p><div className="receipt"><span>Participant name<strong>{form.fullName}</strong></span><span>Registration ID<strong>7D-{Math.floor(10000 + Math.random() * 89999)}</strong></span><span>Payment status<strong className="paid">PAID</strong></span></div><a className="button button-gold full" href="WHATSAPP_GROUP_LINK" onClick={e => e.preventDefault()}>Join WhatsApp workshop group <ArrowUpRight size={17} /></a><p className="fine-print">Please join the WhatsApp group to receive workshop schedule, meeting links, announcements and important updates.</p><button className="receipt-link">Download / view payment receipt</button></div></div>;
 if (flow === 'payment') return <div className="flow-shell"><Logo /><div className="payment-box"><button className="back" onClick={() => onBack()}>← Back to registration</button><p className="eyebrow">Secure checkout · Test mode</p><h1>Complete your<br /><em>enrollment.</em></h1><div className="pay-summary"><span>Seven-day intensive workshop</span><strong><Price /></strong></div><div className="secure-line"><LockKeyhole size={16} /> Razorpay secure hosted checkout · UPI · Cards · Net banking</div><button className="button button-navy full" onClick={onPay}>Continue to Razorpay <ArrowUpRight size={17} /></button><p className="fine-print">Payment verification happens on the server before your seat is marked paid. This demo button represents the Razorpay test checkout.</p></div></div>;
